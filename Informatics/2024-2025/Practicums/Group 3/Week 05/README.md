...
2. Клас `Date` за работа с дати. Класът да съдържа следните методи:
    ...
    * Метод, който извежда информация за датата на стандартния вход.
...
Тео е решил да започне да живее нов живот и най-сетне взе решението да бъде изпълнител в Алфа Мюзик. Помогнете му, като направите програма, която в поредица от балони, наргилета и алкохол, намира трите най-най-скъпи продукта от всеки вид.
...
2. Клас `Playlist`, който се описва с най-много `64` песни и име на изпълнител до `255` символа. Реализирайте следните функции:
...
3. Клас `Application`, който се описва с име, име на производител, както и възможности за съхранение на плейлисти. Имената са символни низове с максимална големина `255` символа. Реализирайте следните функции за приложението:
...
Създайте програма, която има реализиран клас `Point`, съдържащ координатите в Евклидова координатна система. За него да има реализирани следните методи:

* Подразбиращ се конструктор.
* Параметризиран конструктор.
* Селектори на всички полета.
* Мутатори на всички полета.

С помощта на този клас реализирайте клас `Triangle`, който се описва с върховете си, т.е. с три инстанции от тип `Point`. За триъгълника да се пази информация за:

* дължините на страните му;
* радиуса на вписаната окръжност;
* радиуса на описаната окръжност;
* дължините на медианите;
* дължините на ъглополовящите;
* дължините на височините.

С помощта на класа реазилирайте и клас `Circle`, който се описва с центъра и радиуса си. За окръжността да се пази информация за:
...
По въведени координати на точка от стандартния вход да се определи дали тя принадлежи на окръжност и дали принадлежи на триъгълник, чиито данни също се въвеждат от стандартния вход.
...
1. Създайте клас `User`, която има член-данни за име и парола, със съответните методи. Паролата е символен низ с максимална дължина `255` символа. Помислете по какъв начин да имплементирате класа така, че промяната на парола да е невъзможна.
2. Създайте клас `Admin`, който има член данна за име и парола със съответните методи. Администраторът има и ниво на достъп, което не може да се променя - съответно ниво за обикновен потребител и ниво за супер потребител. Паролата е символен низ с максимална дължина `255` символа. Помислете по какъв начин да имплементирате класа така, че промяната на парола да е невъзможна.
3. Създайте клас `System`, който да съдържа колекция от потребители с максимална големина `64` потребителя и точно един администратор. Създайте съответните методи за търсене на потребител в системата, за регистриране на нов потребител в системата, за премахване на потребител от системата. Всеки потребител и всеки администратор трябва да знае в коя система участва. Потребител и администратор могат да вземат участие в точно една система.
4. Създайте клас `Resource`, който ще представлява онлайн ресурс. Онлайн ресурсът се идентифицира с номер цяло число.
5. Създайте клас `Web`, който ще представлява мрежа от ресурси. Броят на ресурсите в мрежата може да бъде максимално `256`. Позволете на класа `Web` да съдържа различни системи в себе си, като добавите поле за връзка от системата към мрежата. Забележете, че мрежата от ресурси може да не бъде единствена в рамките на програмата. Добавете необходимите методи за добавяне, преглеждане и премахване на ресурс от системата.

Преглеждането на ресурс в системата се случва, ако потребителят е аутентикиран, т.е. ако потребителят участва в системата. Веднъж добавен ресурс, то се пази информация кой потребител и добавил ресурса. След това същият ресурс може да бъде премахнат единствено от потребителя, който го е създал, т.е. при ауторизиран потребител. Забележете, че ауторизацията изисква аутентикация, но аутентикацията не изисква ауторизация. Администраторът на една система е ауторизиран винаги.
